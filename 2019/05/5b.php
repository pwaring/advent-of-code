...
while (!$halt)
{
  ...
  elseif ($opcode == 5)
  {
    // If first parameter is non-zero, jump to location in second parameter
    $cmp = $parameter_modes[0] ? $intcodes[$ip + 1] : $intcodes[$intcodes[$ip + 1]];
    $target = $parameter_modes[1] ? $intcodes[$ip + 2] : $intcodes[$intcodes[$ip + 2]];
    $ip = $cmp != 0 ? $target : $ip + 3;
  }
  elseif ($opcode == 6)
  {
    // If first parameter is zero, jump to location in second parameter
    $cmp = $parameter_modes[0] ? $intcodes[$ip + 1] : $intcodes[$intcodes[$ip + 1]];
    $target = $parameter_modes[1] ? $intcodes[$ip + 2] : $intcodes[$intcodes[$ip + 2]];
    $ip = $cmp == 0 ? $target : $ip + 3;
  }
  elseif ($opcode == 7)
  {
    // If first parameter is less than second parameter, store 1 in third parameter, otherwise store 0
    $a = $parameter_modes[0] ? $intcodes[$ip + 1] : $intcodes[$intcodes[$ip + 1]];
    $b = $parameter_modes[1] ? $intcodes[$ip + 2] : $intcodes[$intcodes[$ip + 2]];
    $intcodes[$intcodes[$ip + 3]] = $a < $b ? 1 : 0;

    $ip += 4;
  }
  elseif ($opcode == 8)
  {
    // If first parameter is equal to second parameter, store 1 in third parameter, otherwise store 0
    $a = $parameter_modes[0] ? $intcodes[$ip + 1] : $intcodes[$intcodes[$ip + 1]];
    $b = $parameter_modes[1] ? $intcodes[$ip + 2] : $intcodes[$intcodes[$ip + 2]];
    $intcodes[$intcodes[$ip + 3]] = $a == $b ? 1 : 0;
